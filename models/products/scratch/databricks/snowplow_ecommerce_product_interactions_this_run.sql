...
with prep as (
  ...
  select
    ...
    POSEXPLODE(contexts_com_snowplowanalytics_snowplow_ecommerce_product_1) as (pos, contexts_com_snowplowanalytics_snowplow_ecommerce_product_1)

  from {{ ref('snowplow_ecommerce_base_events_this_run') }} as t

), product_info as (
  select
    {{ dbt_utils.generate_surrogate_key(['event_id', 'contexts_com_snowplowanalytics_snowplow_ecommerce_product_1.id', 'pos']) }} as product_event_id,
    event_id,
    page_view_id,
